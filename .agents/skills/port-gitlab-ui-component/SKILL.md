...
When a Storybook story needs a long block of example copy, use the complete standard Lorem ipsum paragraph already established in `packages/ui/src/base/banner/banner.stories.tsx` (from “Lorem ipsum dolor sit amet” through “mollit anim id est laborum”). Keep product-specific wording only when the wording itself is required to demonstrate behavior or accessibility.

Keep stylesheet provenance boundaries strict. A component's colocated CSS may contain only rules whose semantic source is the corresponding upstream component stylesheet, including declarations generated by mixins, utilities, or required build-compatibility transforms invoked there. Put Bootstrap, vendor, global, or imported transitive rules in the appropriate shared styles layer, scope them to the component marker where necessary, and load that layer before component-private CSS. See [references/react-porting.md](references/react-porting.md) for the detailed boundary.
