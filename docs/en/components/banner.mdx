...
`GlBannerTitle` renders an `h2`. `GlBannerDescription` provides the standard description layout, and `GlBannerActions` lays out custom action controls. All three accept React children, supported attributes for their rendered element, `className`, and refs.
